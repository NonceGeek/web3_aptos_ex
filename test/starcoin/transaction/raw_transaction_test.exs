defmodule Web3AptosEx.Starcoin.Transaction.RawTransactionTest do
  @moduledoc false

  use ExUnit.Case

  alias Web3AptosEx.Starcoin.Account
  alias Web3AptosEx.Starcoin.Transaction.{RawTransaction, TransactionPayload, TypeTag}
  ...
  test "signed data is correct", %{from_address: from_address, to_address: to_address} do
    encode_args = Web3AptosEx.Starcoin.Encoder.encode([to_address.address, 1], [:address, :u128])
    ...
    assert signed_msg == Web3AptosEx.Starcoin.Transaction.signing_message(raw_tx)
  end
