defmodule Web3AptosEx.Aptos.AptosTest do
  @moduledoc false
  use ExUnit.Case, async: true

  setup_all do
     {:ok, rpc} = Web3AptosEx.Aptos.RPC.connect()
     priv = Web3AptosEx.Crypto.generate_priv()
     {:ok, account} = Web3AptosEx.Aptos.Account.from_private_key(priv)
    %{rpc: rpc, ac: account}
  end

  test "call_function", %{rpc: _rpc, ac: account} do
     f = %Web3AptosEx.Aptos.Types.Function{
   address: 0x1,
   is_entry: true,
   module: "coin",
   name: "transfer",
   ...
   _payload = Web3AptosEx.Aptos.call_function(f, ["0x1::aptos_coin::AptosCoin"], [account.address, 100,["abc", "456"]])
   assert _payload=%Web3AptosEx.Aptos.Types.TransactionPayload.EntryFunction{
